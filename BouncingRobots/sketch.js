...
/**
 * Applies contrary force for those 
 */
function calculateSpeed(pos, delta = 50, speed, limit, min = 0)
{

    if((speed > 0 && (pos + delta) >= limit) || speed < 0 && pos <= min)
        return speed * -1;
...
function draw()
{
    ...
    robot2Speed_x = calculateSpeed(robot2_x, 100, robot2Speed_x, width);
    robot2Speed_y = calculateSpeed(robot2_y, 80, robot2Speed_y, height);

    robot1Speed_x = calculateSpeed(robot1_x, 100, robot1Speed_x, width);
    robot1Speed_y = calculateSpeed(robot1_y, 80, robot1Speed_y, height);
